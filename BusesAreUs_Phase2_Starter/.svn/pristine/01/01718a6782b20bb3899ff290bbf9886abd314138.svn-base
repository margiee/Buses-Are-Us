...
public class HttpArrivalDataProvider extends AbstractHttpDataProvider {
    ...
    protected URL getURL() throws MalformedURLException {
        try {
            String arrivalsRequest = "http://api.translink.ca/rttiapi/v1/stops/" +
                    stop.getNumber() +
                    "/estimates?apikey=sukHt6W7S7u3ry1LZNfd";
            return new URL(arrivalsRequest);
        } catch (MalformedURLException e) {
            throw new MalformedURLException("Cannot Produce URL");
        }
    }
